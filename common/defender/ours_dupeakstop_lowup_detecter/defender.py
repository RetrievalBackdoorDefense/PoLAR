...
class OursDuPeakStopLowUpDetecterDefender:
    ...
    def run(self, batch, step=None):
        ...
        peak_step_path = (
            "/data/hust-ls/output/analyze/output-process/process_output/peak_step.json"
        )
        with open(peak_step_path, "r") as f:
            peak_step = json.load(f)[self.dataset_name][self.attack_method]
