...
from defender.ours_lowup_detecter_nograd.detecter import BERTDetecter, GMMDetecter
from defender.ours_lowup_detecter_nograd.smoother import Smoother
...
class OursLowUpDetecterNoGradDefender:
    def __init__(self, **kwargs):
        ...
        self.grad_sims = defaultdict(list)
    ...
    def plot_lowup(self):
        with open(os.path.join(self.lowup_output_dir, "grad-sims.json"), "w") as f:
            f.write(json.dumps(self.grad_sims, indent=4))
    ...
    def update_lowup(self, upper_resp, lower_resp, curr_resp, curr_batch):
        if len(self.upper_smaples) == 0 and len(self.lower_smaples) == 0:
            ...
            lower_sim = F.cosine_similarity(
                curr_resp["grad"], lower_resp["grad"], dim=0
            ).item()
            upper_sim = F.cosine_similarity(
                curr_resp["grad"], upper_resp["grad"], dim=0
            ).item()
        ...
        self.grad_sims["lowup"].append(lowup_sim)
        self.grad_sims["upper"].append(upper_sim)
        self.grad_sims["lower"].append(lower_sim)
        ...
        self.smoother.update(lowup_sim)
    ...
    def run(self, batch, step=None):
        ...
        preds = curr_preds = [0] * len(curr_resp["losses"])
