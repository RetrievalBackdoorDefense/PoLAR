...
class OursPilotLowUpDetecterDefender:
    def __init__(self, **kwargs):
        ...
        self.upper_smaples_map = {}
        self.upper_metric_path_map = {}
        for k in range(1, self.lowup_sample_capacity + 1):
            self.upper_smaples_map[k] = []
            self.upper_metric_path_map[k] = os.path.join(
                self.lowup_output_dir, f"upper-{k}-metric.jsonl"
            )
            with open(self.upper_metric_path_map[k], "w") as f:
                pass

        self.lower_smaples_map = {}
        self.lower_metric_path_map = {}
        for k in range(1, self.lowup_sample_capacity + 1):
            self.lower_smaples_map[k] = []
            self.lower_metric_path_map[k] = os.path.join(
                self.lowup_output_dir, f"lower-{k}-metric.jsonl"
            )
            with open(self.lower_metric_path_map[k], "w") as f:
                pass
    ...
    def plot_lowup(self):
        for way in ["upper", "lower"]:
            if way == "upper":
                ...
                metric_path_map = self.upper_metric_path_map
            else:
                metric_key = "lower-top{}-cr"
                metric_path_map = self.lower_metric_path_map

            metrics = defaultdict(list)
            for k in range(1, self.lowup_sample_capacity + 1):
                with open(metric_path_map[k], "r") as f:
                    for line in f:
    ...
    def update_lowup(self, upper_resp, lower_resp, curr_resp, curr_batch):
        for k in range(1, self.lowup_sample_capacity + 1):
            if (
                len(self.upper_smaples_map[k]) == 0
                and len(self.lower_smaples_map[k]) == 0
            ):
                whole_samples = [
                    {
                ...
                whole_samples = (
                    [
                        ...
                        for i, json_obj in enumerate(self.upper_smaples_map[k])
                    ]
                    + [
                        {
                            "sample": json_obj["sample"],
                        ...
                        for i, json_obj in enumerate(self.lower_smaples_map[k])
                    ]
                    + [
                        {
                            "sample": get_item(curr_batch, i),
            ...
            self.upper_smaples_map[k] = whole_samples[:k]
            self.lower_smaples_map[k] = whole_samples[-k:]

            self.upper_smaples_map[k] = sorted(
                self.upper_smaples_map[k], key=lambda x: -x["loss"]
            )
            self.lower_smaples_map[k] = sorted(
                self.lower_smaples_map[k], key=lambda x: x["loss"]
            )

            upper_metric_json = {}
            for _k in range(1, len(self.upper_smaples_map[k]) + 1):
                poison_labels = [
                    json_obj["poison_label"]
                    for json_obj in self.upper_smaples_map[k][:_k]
                ]
                upper_metric_json[f"upper-top{_k}-pr"] = round(
                    sum(poison_labels) / len(poison_labels) * 100, 4
                )

            lower_metric_json = {}
            for _k in range(1, len(self.lower_smaples_map[k]) + 1):
                poison_labels = [
                    json_obj["poison_label"]
                    for json_obj in self.lower_smaples_map[k][:_k]
                ]
                lower_metric_json[f"lower-top{_k}-cr"] = round(
                    (len(poison_labels) - sum(poison_labels))
                    / len(poison_labels)
                    * 100,
                    4,
                )

            with open(self.upper_metric_path_map[k], "a") as f:
                f.write(json.dumps(upper_metric_json) + "\n")

            with open(self.lower_metric_path_map[k], "a") as f:
    ...
    def run(self, batch, step=None):
        ...
        if (
            len(self.upper_smaples_map[self.lowup_sample_capacity]) == 0
            and len(self.lower_smaples_map[self.lowup_sample_capacity]) == 0
        ):
            ...
            if (
                ...
                upper_resp = self.grad_collector.grad(
                    batch=merge_item(
                        [
                            json_obj["sample"]
                            for json_obj in self.upper_smaples_map[
                                self.lowup_sample_capacity
                            ]
                        ]
                    )
                )
                ...
                lower_resp = self.grad_collector.grad(
                    batch=merge_item(
                        [
                            json_obj["sample"]
                            for json_obj in self.lower_smaples_map[
                                self.lowup_sample_capacity
                            ]
                        ]
                    )
        ...
        if self.action["train-detecter"]:
            texts = [
                ...
                for json_obj in self.upper_smaples_map[self.lowup_sample_capacity][
                    : self.lowup_train_batch_size
                ]
            ] + [
                json_obj["sample"].query
                for json_obj in self.lower_smaples_map[self.lowup_sample_capacity][
                    : self.lowup_train_batch_size
                ]
            ]
