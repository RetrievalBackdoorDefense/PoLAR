...
from OpenAttack.text_process.constituency_parser import (
    ConstituencyParser,
    get_default_constituency_parser,
)
...
from OpenAttack.attackers.classification import (
...
import models
from PositiveFeedback.common.poisoner.hidden_killer.scpn import subword
...
class SCPNAttacker(ClassificationAttacker):
    ...
    def __init__(
        ...
        parse_model = torch.load(
            model_path["parse_generator.pt"], map_location=self.device
        )
        ...
        self.net = models.SCPN(
            pp_args.d_word,
            pp_args.d_hid,
            pp_args.d_nt,
            pp_args.d_trans,
            len(self.pp_vocab),
            len(self.parse_gen_voc) - 1,
            pp_args.use_input_parse,
        )
        ...
        self.parse_net = models.ParseNet(
    ...
    def gen_paraphrase(self, sent, templates):
        ...
        for z, template in enumerate(templates):
            np_templates[z, : template_lens[z]] = [
                self.parse_gen_voc[w] for w in templates[z].split()
            ]
        ...
        torch_parse = torch.LongTensor([self.parse_gen_voc[w] for w in parse_tree]).to(
            self.device
        )
        ...
        beam_dict = self.parse_net.batch_beam_search(
            torch_parse.unsqueeze(0),
            tp_templates,
            torch_parse_len[:],
            tp_template_lens,
            self.parse_gen_voc["EOP"],
            beam_size=3,
            max_steps=150,
        )
        seq_lens = []
